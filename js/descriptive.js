...
// 画GDP的div，加载CSV数据
d3.csv('../data/data_gdp.csv').then(data => {
    ...
    const yAxis = g => g
        ...
        .call(d3.axisLeft(y).ticks(6).tickFormat(d => (d / 1e8).toFixed(0)));
    // 添加图表标题
    svg.append('text')
        .attr('class', 'chart-title')
        .attr('x', width / 2)
        .attr('y', margin.top - 10)  // 在图表上方添加标题
        .attr('text-anchor', 'middle')
        .style('font-size', '16px')
        .style('font-weight', 'bold')
        .text('各国1990-2019年GDP变化折线图');
    ...
    svg.append('text')
        ...
        .attr('x', width / 2)
        .attr('y', height - margin.bottom + 40)  // 调整位置
        .attr('text-anchor', 'middle')
        .style('font-size', '14px')
        .text('年份');
